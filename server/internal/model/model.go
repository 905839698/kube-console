...
type LogSource struct {
	...
	IndexPrefix string `gorm:"size:128" json:"indexPrefix"`        // 行日志索引前缀（stdout 与单行文本文件采集），如 k8s-
	// JSON 结构化采集写入的索引前缀，与行日志分索引，避免任意字段污染行日志 mapping（空=logstash-）
	JsonIndexPrefix string `gorm:"size:128;default:logstash-" json:"jsonIndexPrefix"`
