  ...
  routes: [
    { path: '/login', name: 'login', component: () => import('../views/Login.vue'), meta: { title: '登录' } },
    {
      ...
      children: [
        ...
        { path: 'clusters', name: 'clusters', component: () => import('../views/ClusterList.vue'), meta: { title: '集群管理', admin: true, platformView: true } },
...
{ path: 'ci', name: 'ci', component: () => import('../views/CIPipelines.vue'), meta: { title: 'CI 流水线' } },
        ...
        // 平台管理（platform-admin 全量；标了 platformView 的对 platform-viewer 只读开放）
        { path: 'users', name: 'users', component: () => import('../views/UserList.vue'), meta: { title: '用户管理', admin: true } },
        { path: 'authz', name: 'authz', component: () => import('../views/AuthzList.vue'), meta: { title: '授权管理', admin: true, platformView: true } },
        { path: 'roles', name: 'roles', component: () => import('../views/RoleList.vue'), meta: { title: '角色管理', admin: true, platformView: true } },
        { path: 'audit', name: 'audit', component: () => import('../views/AuditList.vue'), meta: { title: '审计日志', admin: true, platformView: true } },
        ...
        { path: 'usage', name: 'usage', component: () => import('../views/UsageReport.vue'), meta: { title: '用量报表', admin: true, platformView: true } },
...
router.beforeEach(async (to, from) => {
  ...
  // 平台管理页面：platform-admin 全量；platform-viewer 仅可进只读页（授权/角色/审计/报表）
  if (to.meta.admin && !user.isAdmin && !(to.meta.platformView && user.viewer)) {
